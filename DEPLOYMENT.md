...
> 目标：让 `nizhenghang-homepage` 长期在线，任何人可通过公网 URL 访问，且后续代码更新能自动部署。
>
...
2. 填写信息：
   - **Repository name**：`nizhenghang-homepage`
...
4. 创建后页面会显示仓库地址，形如：
   ```
   https://github.com/Nizhenghang/nizhenghang-homepage.git
   ```
   记下这个地址，下一步要用。
...
本地仓库已初始化并完成首次提交（commit `372cc96`，包含 7 个核心文件）。

在项目根目录打开终端（Git Bash / PowerShell 均可），执行：

```bash
# 关联远程仓库（替换为你的实际仓库地址）
git remote add origin https://github.com/Nizhenghang/nizhenghang-homepage.git
...
| **SSH Key** | 长期使用，推荐 | `ssh-keygen -t ed25519 -C "你的邮箱"` → 将 `~/.ssh/id_ed25519.pub` 添加到 GitHub Settings → SSH and GPG keys → 改用 `git remote set-url origin git@github.com:Nizhenghang/nizhenghang-homepage.git` |
| **GitHub CLI** | 已安装 gh CLI | `gh auth login` 后推送无需额外认证 |

推送成功后，访问 `https://github.com/Nizhenghang/nizhenghang-homepage` 应能看到所有文件。
...
3. 关联 GitHub：
   ...
   - 选择 `Nizhenghang/nizhenghang-homepage` 仓库，点击 **Install**
...
```
https://nizhenghang-homepage-xxxxxx.eo.app
```
